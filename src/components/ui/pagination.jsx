...
function getPaginationItems(
    ...
    // 모바일에서는 더 적은 페이지 표시
    const effectiveDelta = isMobile ? 0 : delta;
    const effectiveMaxAllPages = isMobile ? 5 : maxAllPages;

    if (totalPages <= effectiveMaxAllPages) {
    ...
    set.add(1);
    set.add(totalPages);
    for (let p = currentPage - effectiveDelta; p <= currentPage + effectiveDelta; p++) {
...
function PaginationBar({
    ...
    const items = getPaginationItems(page, totalPages, delta, maxAllPages, isMobile);
    ...
    return (
        <Pagination className={className}>
            <div className="flex flex-col items-center gap-2">
                ...
                {/* 처음/끝 버튼 */}
                {showFirstLast ? (
                    <div className="flex md:flex-row justify-center w-full gap-1 sm:gap-2">
                        <PaginationFirst
                            href={getPageHref(1)}
                            className={cn(
                                firstDisabled &&
