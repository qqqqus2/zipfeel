 ...
 * - variant: "modal" | "absolute" - 팝업 형태 (modal: dim 처리된 LayerPopup, absolute: 일반 위치 팝업)
 */
export const CharacterPopup = ({ open, onOpenChange, variant = "modal" }) => {
    ...
    if (variant === "absolute") {
        ...
        return (
            <div className="w-full h-full overflow-hidden flex-1 flex flex-col mt-[15px] absolute z-10 bg-white">
                {/* 헤더 영역 */}
                <div className="bg-gray-6 relative pt-[78px] pb-[52px] px-5 pt-5">
                    <button
                        type="button"
                        aria-label="close"
                ...
                <div className="relative w-[calc(100%-15px)] left-[15px]">
                    ...
                    >
                        <TabsList className="w-full justify-start gap-3">
                            <TabsTrigger
                                value="history"
                                className="px-5 min-w-[80px] shrink-0 !fz-14 h-[42px]"
                            >
                                히스토리
                            </TabsTrigger>
                            <TabsTrigger
